  ...
  : G4VPhysicsConstructor("nEXOG4EmLivermorePhysics"), verbose(ver)
{
  ...
  G4cout << "using nEXOG4EmLivermorePhysics" << G4endl;
}
...
void nEXOG4EmLivermorePhysics::ConstructProcess()
{
  ...
  while( (*aParticleIterator)() ){
    ...
    if(particleName != "gamma" && particleName != "opticalphoton"){
        G4StepLimiter* eStepLim = new G4StepLimiter();
        ph->RegisterProcess(eStepLim, particle);
    }
  ...
  de->SetFluo(true);
  de->SetAuger(true);
  G4cout << "nEXOG4EmLivermorePhysics: Auger physics is on" << G4endl; 
}
